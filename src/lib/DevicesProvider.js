...
export class DeviceProvider extends Component {
  ...
  async componentDidMount() {
    const dev = await this.getDevicesFromServer()
  ...
  onSaveHandler = R.curry((id, item) => {
    const { devices } = this.state
    const newDevice = this.updateDeviceOnServer(id, item)
  ...
  onChangeActive = id => {
    ...
    this.setState({ devices: newDevices })
    this.updateDeviceOnServer(id, newDevices[deviceFoundIndex])
  }

  getDevicesFromServer = async (offset, limit) => {
    const response = await read(`http://localhost/api/devices?${objectToUrl({
      offset,
      limit,
    })}`)
  ...
  deleteDeviceFromServer = async id => {
    const response = await destroy(`http://localhost/api/devices/remove/${id}`)
    return response
  }

  updateDeviceOnServer = async (id, item) => {
  ...
  deleteHandler = async id => {
    const { devices } = this.state
    await this.deleteDeviceFromServer(id)
